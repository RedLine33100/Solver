...
public abstract class Calcul<T extends Number> extends ValueGetter<T> {
    ...
    public Calcul(ValueGetter<T> previous, Pair<BinaryOperator<T>, ValueGetter<T>> operator) {
        ...
        this.addAllUnknownVars(previous.getUnknownVariables());
        if(operator != null)
            this.addAllUnknownVars(operator.r().getUnknownVariables());
    }
