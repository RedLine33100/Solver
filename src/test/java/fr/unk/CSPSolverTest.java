...
class CSPSolverTest {
    ...
    void testSolveStructurel() {
        // Cas où les variables inconnues et les contraintes sont vides :
        // vérifie si la méthode solve() renvoie null dans ce cas
        CSPSolver<Integer> solver1 = new CSPSolver<>();
        assertNull(solver1.solve(new ArrayList<> (), new HashMap<> ()));

        // Cas où une seule variable avec une containte :
        // vérifie si la méthode solve() renvoie la bonne solution dans ce cas
        ...
        // Cas où deux variables avec une contrainte et une valeur compatible :
        // vérifie si la méthode solve() renvoie la bonne solution dans ce cas
        Variable<Integer> var3 = new Variable<>("z", Integer.class);
        solver1.addConstraint(new Equals<> (var3, 5));
        IntDomain domain3 = new IntDomain(0, 10, 1);
        solver1.addUnknownVariable (var1, domain3);
        solver1.addUnknownVariable(var3, domain3);
        Map<String, Object> solution3 = solver1.trySolve();
        assertNotNull(solution3);
        assertEquals(2, solution3.size());
        assertEquals(5, solution3.get("x"));
